...
let initialTargetHash =
  "000000000000000000000000000000000ffffffffffffffffffffffffff"; //                  Integer: 20282409603651670423947251286015
...
let calculateTargetHash = (index) => {
  ...
  if (actualHashRate > desiredHashRate) {
    console.log(
      "** Higher Hash Rate **, Decreasing Target Hash to " +
        Math.round((desiredHashRate / actualHashRate) * 100) / 100 +
        "% of " +
        lastBlockTargetHash
    );
    return hexMultiplication(
      lastBlockTargetHash,
      desiredHashRate / actualHashRate
    );
  } else if (actualHashRate < desiredHashRate) {
    console.log(
      "** Lower Hash Rate **, Increasing Target Hash to " +
        Math.round((desiredHashRate / actualHashRate) * 100) / 100 +
        "% of " +
        lastBlockTargetHash
    );
  ...
  } else return lastBlockTargetHash;
};
...
let createBlock = () => {
  return new Promise((resolve) => {
    ...
    while (parseInt(newBlock.hash, 16) > parseInt(newTargetHash, 16)) {
      // Step 3
...
function main() {
  function loop() {
    return createBlock().then(function (newBlock) {
      blockchain.push(newBlock);
      console.log(
        "Mined block in " +
          (newBlock.timestamp - blockchain[newBlock.index - 1].timestamp) /
            1000 +
          " sec => ",
        newBlock
      );
